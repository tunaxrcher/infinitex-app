...
export function AgentLoanApplicationFlow() {
  ...
  useEffect(() => {
    ...
    console.log(
      '[AgentFlow] Loading from sessionStorage:',
      savedData,
      'step:',
      savedStep
    )
  ...
  useEffect(() => {
    ...
    const dataToSave = {
      ...
      titleDeeds: applicationData.titleDeeds?.map(
        ({ imageFile, ...rest }) => rest
      ),
    }
    ...
    console.log('[AgentFlow] Saved to sessionStorage:', {
      deedMode: applicationData.deedMode,
      currentStep,
    })
